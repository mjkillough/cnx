...
impl Widget for Volume {
    fn stream(self: Box<Self>) -> WidgetStream {
        ...
        let mixer = Mixer::new(mixer_name, true).unwrap();
        let stream = AlsaEventStream::new(self.handle.clone(), mixer)
            .map(move |()| {
                ...
                let mute = master.get_playback_switch(channel).unwrap() == 0;

                let text = if !mute {
                    let volume = master.get_playback_volume(channel).unwrap();
...
impl AlsaEventStream {
    fn new(handle: Handle, mixer: Mixer) -> AlsaEventStream {
        AlsaEventStream {
            poll: PollEvented::new(AlsaEvented(mixer), &handle).unwrap(),
